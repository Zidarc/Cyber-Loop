...
/* ─────────────────────────────────────────
   CANVAS: embers + spores + mind-flayer web
───────────────────────────────────────── */
function SceneCanvas({ active }) {
  ...
  useEffect(() => {
    ...
    const PALETTE = [
      ...
      [255,120,0],[180,20,0],[255,160,40],
    ]

    function spawnEmber(W, H) {
      const rgb = PALETTE[Math.floor(Math.random()*PALETTE.length)]
      const size = 0.4 + Math.random()*1.2        // tiny: 0.4–1.6px
      const travel = H*(0.50+Math.random()*0.10)
      return {
        x:Math.random()*W, y:H,
        size, vx:(Math.random()-0.5)*0.28,
        vy:-(0.6+Math.random()*1.0),              // faster
        travel, dist:0, rgb,
        wobble:Math.random()*Math.PI*2,
        wobbleSpd:0.015+Math.random()*0.022,
      }
    }

    // Upside Down spores — drift sideways like ash
    ...
    const spores = Array.from({length:SPORE_COUNT},()=>{
      const s=spawnSpore(W0,H0)
      s.x=Math.random()*W0
      return s
    })

    // Mind-Flayer web nodes
    let webNodes=[]
    function buildWeb(W,H){
      webNodes=Array.from({length:20},()=>({
    ...
    function render(){
      ...
      // Vignette only — no orange blob
      const bv=0.56+Math.sin(t*0.016)*0.05
      const vig=ctx.createRadialGradient(W/2,H/2,H*0.08,W/2,H/2,H*0.88)
      vig.addColorStop(0,'rgba(0,0,0,0)')
      vig.addColorStop(0.6,`rgba(0,0,0,${bv*0.20})`)
      vig.addColorStop(1,`rgba(0,0,0,${bv})`)
      ctx.fillStyle=vig; ctx.fillRect(0,0,W,H)

      // Thin floor warmth strip only
      const fa=0.07+Math.sin(t*0.04)*0.03
      const fl=ctx.createLinearGradient(0,H*0.90,0,H)
      fl.addColorStop(0,'rgba(0,0,0,0)')
      fl.addColorStop(1,`rgba(140,35,0,${fa})`)
      ctx.fillStyle=fl; ctx.fillRect(0,H*0.90,W,H*0.10)
      ...
      for(let i=0;i<webNodes.length;i++){
        for(let j=i+1;j<webNodes.length;j++){
          ...
          ctx.strokeStyle=`rgba(160,45,0,${fade*0.09})`
          ctx.lineWidth=fade*0.7; ctx.stroke()
        }
      }
      for(const n of webNodes){
        const al=0.10+Math.sin(n.pulse)*0.05
        ctx.beginPath(); ctx.arc(n.x,n.y,n.r,0,Math.PI*2)
        ctx.fillStyle=`rgba(200,70,0,${al})`; ctx.fill()
      }
      ctx.restore()
      ...
      for(let i=0;i<spores.length;i++){
        const s=spores[i]
        s.x+=s.vx; s.y+=s.vy; s.wobble+=s.wobbleSpd; s.pulse+=s.pulseSpd
        s.y+=Math.sin(s.wobble)*0.07
        if((s.vx>0&&s.x>W+12)||(s.vx<0&&s.x<-12)){
          Object.assign(s,spawnSpore(W,H))
        }
        const pa=s.alpha*(0.72+Math.sin(s.pulse)*0.28)
        ctx.save()
        ctx.globalAlpha=pa
        // crosshair spore
        ctx.strokeStyle=`rgba(255,130,35,${pa*0.55})`
        ctx.lineWidth=0.45
        ctx.beginPath(); ctx.moveTo(s.x-s.r*1.9,s.y); ctx.lineTo(s.x+s.r*1.9,s.y); ctx.stroke()
        ctx.beginPath(); ctx.moveTo(s.x,s.y-s.r*1.9); ctx.lineTo(s.x,s.y+s.r*1.9); ctx.stroke()
        // glow center
        const sg=ctx.createRadialGradient(s.x,s.y,0,s.x,s.y,s.r*2.8)
        sg.addColorStop(0,`rgba(255,170,50,${pa})`)
        sg.addColorStop(0.4,`rgba(210,75,0,${pa*0.65})`)
        sg.addColorStop(1,'rgba(0,0,0,0)')
        ctx.fillStyle=sg; ctx.beginPath(); ctx.arc(s.x,s.y,s.r*2.8,0,Math.PI*2); ctx.fill()
        ctx.restore()
      }
      ...
      for(let i=0;i<embers.length;i++){
        ...
        if(e.dist>=e.travel){
          Object.assign(e,spawnEmber(W,H)); e._delay=0; e._ticks=0; continue
        }
        ...
        const alpha=prog<0.05
          ? prog/0.05
          : prog>0.58 ? Math.pow(1-(prog-0.58)/0.42,1.9) : 1
        if(alpha<0.01) continue

        const [r,g,b]=e.rgb
        const bodyR=e.size*(1-prog*0.22)

        ctx.save()
        ctx.globalAlpha=alpha
        ctx.translate(e.x,e.y); ctx.rotate(e.wobble*0.45)
        const s=bodyR
        ctx.beginPath()
        ...
        ctx.fillStyle=`rgba(${Math.floor(r*0.20)},${Math.floor(g*0.05)},0,1)`
        ctx.fill()
        ctx.save(); ctx.clip()
        const inner=ctx.createRadialGradient(s*0.05,-s*0.1,0,s*0.05,-s*0.1,s*0.9)
        inner.addColorStop(0,'rgba(255,245,190,1)')
        inner.addColorStop(0.22,`rgba(${r},${Math.max(g,28)},0,1)`)
        inner.addColorStop(0.62,`rgba(${Math.floor(r*0.52)},0,0,1)`)
        inner.addColorStop(1,'rgba(0,0,0,0)')
        ctx.fillStyle=inner; ctx.fill()
        ctx.restore(); ctx.restore()

        if(prog<0.68){
          ctx.save(); ctx.globalAlpha=alpha*0.13
          const tl=bodyR*(2.2+prog*3)
          const trail=ctx.createLinearGradient(e.x,e.y,e.x+e.vx*2,e.y+tl)
          trail.addColorStop(0,`rgba(${r},${Math.max(g-20,0)},0,1)`)
          trail.addColorStop(1,'rgba(0,0,0,0)')
          ctx.strokeStyle=trail; ctx.lineWidth=bodyR*0.32
          ctx.beginPath(); ctx.moveTo(e.x,e.y); ctx.lineTo(e.x+e.vx*2,e.y+tl); ctx.stroke()
          ctx.restore()
        }
      }
    }
  ...
  return (
    <canvas ref={ref} style={{position:'absolute',inset:0,width:'100%',height:'100%',zIndex:2,pointerEvents:'none'}}/>
  )
}

/* ─────────────────────────────────────────
   CHRISTMAS LIGHT STRIP — Stranger Things
───────────────────────────────────────── */
const LIGHT_COLORS=['#FF4500','#FF8C00','#FFD060','#FF2200','#FF6A00','#FFAA00','#FF5500','#FFC040']
function LightStrip(){
  const [states,setStates]=useState(()=>
    Array.from({length:20},(_,i)=>({color:LIGHT_COLORS[i%LIGHT_COLORS.length],on:true,bri:1}))
  )
  useEffect(()=>{
    const iv=setInterval(()=>{
      setStates(prev=>prev.map(l=>({
        ...l,
        on:Math.random()>0.10,
        bri:0.6+Math.random()*0.4,
      })))
    },110)
      ...
      {states.map((l,i)=>(
        <div key={i} style={{display:'flex',flexDirection:'column',alignItems:'center'}}>
          ...
          <div style={{
            ...
            background:l.on?l.color:'rgba(25,8,0,0.85)',
            boxShadow:l.on?`0 0 ${5*l.bri}px ${l.color},0 0 ${13*l.bri}px ${l.color}90`:'none',
            opacity:l.on?(0.7+l.bri*0.3):0.22,
...
export default function Login(){
  ...
  async function handleSubmit(e){
    ...
    setError('')
    if(!tn||!pw){
      setShake(true); setError('Team name and password are required.')
      setTimeout(()=>setShake(false),420); return
    }
    setLoading(true)
    try{
      await sleep(800)
      ...
      setError('Authentication failed. Try again.'); setLoading(false)
    }
  }
  ...
  return (
    <div style={{minHeight:'100vh',position:'relative',overflow:'hidden',display:'flex',alignItems:'center',justifyContent:'center',background:'#060100',userSelect:'none',WebkitUserSelect:'none'}}>
      <style>{`
        @import url('https://fonts.googleapis.com/css2?family=Libre+Baskerville:wght@700&family=Share+Tech+Mono&family=Barlow:wght@400;600&display=swap');
        *{user-select:none;-webkit-user-select:none;box-sizing:border-box}
        ...
        @keyframes stFlicker{
          0%,100%{opacity:1} 93.5%{opacity:1} 94%{opacity:0.55} 94.3%{opacity:1} 97.8%{opacity:0.82} 98%{opacity:1}
        }
        .st-title{
          position:relative;display:inline-block;
          animation:stG 14s steps(1) infinite,stFlicker 8s linear infinite;
        }
        .st-title::before,.st-title::after{
          content:attr(data-text);position:absolute;inset:0;
          background:linear-gradient(180deg,#FF9500 0%,#FF4800 42%,#CC1200 100%);
        ...
        @keyframes noiseShift{
          ...
          50%{background-position:-9px 14px} 75%{background-position:6px -11px} 100%{background-position:0 0}
        }
        ...
        .hell-input{
          width:100%;background:rgba(4,1,0,0.68);
          border:1px solid rgba(95,32,0,0.52);border-radius:7px;
          padding:11px 14px;color:#F5ECD7;
          font-family:'Share Tech Mono',monospace;font-size:0.88rem;
          outline:none;caret-color:#FF7700;
          transition:border-color .2s,box-shadow .2s,background .2s;
        }
        .hell-input::placeholder{color:rgba(240,195,130,0.18);font-style:italic}
        .hell-input:focus{
          border-color:rgba(215,95,0,0.82);background:rgba(6,2,0,0.80);
          box-shadow:0 0 0 3px rgba(195,75,0,0.12),0 0 16px rgba(190,65,0,0.09);
        }
        .login-btn{
          width:100%;padding:13px 0;border:none;border-radius:7px;
          font-family:'Libre Baskerville',Georgia,serif;
          font-size:0.80rem;font-weight:700;letter-spacing:0.24em;
          text-transform:uppercase;cursor:pointer;position:relative;
          overflow:hidden;transition:filter .2s,transform .15s;
        }
        .btn-on{
          background:linear-gradient(180deg,#FF8A00 0%,#CC5000 45%,#881000 100%);
          color:#fff;text-shadow:0 1px 5px rgba(0,0,0,0.6);
        ...
        .btn-off{
          background:rgba(255,255,255,0.04);color:rgba(240,195,130,0.16);
          cursor:not-allowed;border:1px solid rgba(75,28,0,0.22);
        }
      `}</style>
      ...
      <video autoPlay loop muted playsInline
        ...
        style={{position:'absolute',inset:0,width:'100%',height:'100%',objectFit:'cover',zIndex:0,opacity:videoReady?0.76:0,transition:'opacity 1.2s ease'}}
      >
      ...
      <div
        ...
        style={{
          position:'relative',zIndex:10,width:'min(92vw,415px)',
          background:'rgba(6,2,0,0.52)',borderRadius:'11px',
          border:'1px solid rgba(150,50,0,0.36)',
          backdropFilter:'blur(7px)',
          padding:'clamp(1.5rem,4vw,2.1rem) clamp(1.3rem,4vw,2rem)',
          overflow:'hidden',
        }}
      >
        <VineBorder/>
        ...
        {/* Noise overlay */}
        <div style={{
          position:'absolute',inset:0,pointerEvents:'none',zIndex:0,borderRadius:'11px',
          ...
          backgroundSize:'175px',opacity:0.025,
          animation:'noiseShift 0.45s steps(1) infinite',mixBlendMode:'overlay',
        }}/>
        ...
        <div style={{position:'relative',zIndex:2}}>
          ...
          <div style={{textAlign:'center',marginBottom:'1.2rem',marginTop:'2px'}}>
            <p style={{fontFamily:'"Share Tech Mono",monospace',fontSize:'0.58rem',letterSpacing:'0.30em',textTransform:'uppercase',color:'rgba(235,150,70,0.38)',marginBottom:'0.55rem'}}>
              // do you copy?
            </p>

            <h1
              className="st-title"
              data-text="RECURSION HELL"
              style={{
                fontFamily:'"Libre Baskerville",Georgia,"Times New Roman",serif',
                fontSize:'clamp(1.55rem,5.5vw,2.05rem)',fontWeight:700,
                letterSpacing:'0.060em',lineHeight:1.06,margin:0,
                background:'linear-gradient(180deg,#FFB040 0%,#FF6500 36%,#FF2200 74%,#961000 100%)',
            ...
            <p style={{fontFamily:'"Share Tech Mono",monospace',fontSize:'0.56rem',color:'rgba(215,120,45,0.32)',letterSpacing:'0.20em',marginTop:'0.48rem'}}>
              THE &nbsp; UPSIDE &nbsp; DOWN &nbsp; ∞
            </p>
          </div>
