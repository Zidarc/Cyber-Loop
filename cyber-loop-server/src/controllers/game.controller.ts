...
//GET /api/game/question/:questionId/files
export async function getQuestionFiles(req: Request, res: Response): Promise<void> {
  const user = (req as Request & { user?: JwtPayload }).user;
  if (!user?.participantId) {
    res.status(401).json({ error: 'Unauthorized' });
    return;
  ...
  const result = await gameService.getQuestionFiles(user.participantId, questionId);

  if (!result.ok) {
    if (result.status === 'not_found')     { res.status(404).json({ error: 'Question or file not found' }); return; }
    if (result.status === 'forbidden')     { res.status(403).json({ error: 'Forbidden' }); return; }
    if (result.status === 'invalid_path')  { res.status(403).json({ error: 'Forbidden' }); return; }
